...
export default function QCM({ idea, questions, index, onBack, onAnswer }) {
    ...
    <div style={{ height: '100dvh', display: 'flex', flexDirection: 'column', background: 'var(--bg)', position: 'relative', overflow: 'hidden', animation: 'gn-fadein .35s ease', paddingTop: 'calc(env(safe-area-inset-top, 14px) + 16px)' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '18px 24px 24px', position: 'relative', zIndex: 2 }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
          {q.opts.map((opt) => (
            <button key={opt.label} onClick={() => onAnswer(opt.label)} style={{ background: '#fff', borderRadius: 24, padding: '18px 12px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10, boxShadow: '0 8px 18px -12px rgba(74,58,102,.32)' }}>
              {opt.color ? (
