...
export default function Premium({ onBack, onSubscribe, onOpenReader }) {
  return (
    <div style={{ height: '100dvh', display: 'flex', flexDirection: 'column', background: 'var(--bg)', position: 'relative', overflow: 'hidden', animation: 'gn-fadein .35s ease', paddingTop: 'calc(env(safe-area-inset-top, 14px) + 16px)' }}>
      ...
      <div style={{ padding: '6px 24px 10px', display: 'flex', alignItems: 'center', gap: 12, position: 'relative', zIndex: 2 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'linear-gradient(135deg,#FFD86A,#FFB23F)', color: '#7a4a00', fontSize: 13, fontWeight: 700, padding: '7px 12px', borderRadius: 20, boxShadow: '0 6px 14px -4px rgba(255,160,40,.6)', flex: 'none' }}><RawSvg html={crownBadge} />Premium</div>
      </div>

      <button onClick={() => onOpenReader('weekly')} style={{ textAlign: 'left', margin: '4px 24px 0', borderRadius: 32, padding: 20, background: 'linear-gradient(150deg,#FFC9E0,#D9C4FF)', position: 'relative', zIndex: 2, boxShadow: '0 14px 30px -12px rgba(169,140,255,.6)' }}>
        <div style={{ fontSize: 13, fontWeight: 700, color: '#9b3f76' }}>✨ Nouvelle cette semaine</div>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 8 }}>
          <div>
            <div style={{ fontSize: 20, fontWeight: 700, lineHeight: 1.12, maxWidth: 160 }}>Le château dans les nuages</div>
      ...
      <div style={{ flex: 1, padding: '18px 24px 10px', position: 'relative', zIndex: 2 }}>
        <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--ink2)', marginBottom: 12 }}>Bientôt débloquées</div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
          {[0, 1, 2, 3].map((i) => (
            <button key={i} onClick={onSubscribe} style={{ background: '#F0ECF6', borderRadius: 26, height: 120, position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <div style={{ position: 'absolute', top: 10, right: 10, background: '#A98CFF', color: '#fff', fontSize: 11, fontWeight: 700, padding: '4px 9px', borderRadius: 14 }}>Premium</div>
              <RawSvg html={lock} />
            </button>
          ))}
        </div>
