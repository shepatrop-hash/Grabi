...
const iconHome = `<svg width="24" height="24" viewBox="0 0 24 24"><path d="M4 11.5 L12 4.5 L20 11.5 V20 H4 Z" fill="#fff"></path></svg>`
const iconCommunity = `<svg width="30" height="30" viewBox="0 0 40 40"><circle cx="20" cy="20" r="15" fill="none" stroke="#C3BBD2" stroke-width="2.6"></circle><circle cx="15" cy="19" r="2.2" fill="#C3BBD2"></circle><circle cx="25" cy="19" r="2.2" fill="#C3BBD2"></circle><path d="M15,25 Q20,29 25,25" stroke="#C3BBD2" stroke-width="2.2" fill="none" stroke-linecap="round"></path></svg>`
const iconPlus = `<svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#C3BBD2" stroke-width="2.2" stroke-linecap="round"><path d="M12 5 V19 M5 12 H19"></path></svg>`
const iconBookmark = `<svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="#C3BBD2" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M7 4 H17 V20 L12 16 L7 20 Z"></path></svg>`

const whiteCircle = {
  width: 80, height: 80, borderRadius: '50%', background: '#fff',
  display: 'flex', alignItems: 'center', justifyContent: 'center', flex: 'none',
}
const navBtn = { width: 46, height: 46, display: 'flex', alignItems: 'center', justifyContent: 'center' }
...
export default function Home({ onGoFree, onGoPremium, onGoCreate, onGoCommunity, onGoMine }) {
  return (
    <div style={{ height: '100dvh', display: 'flex', flexDirection: 'column', background: 'var(--bg)', position: 'relative', overflow: 'hidden', animation: 'gn-fadein .35s ease', paddingTop: 'env(safe-area-inset-top, 14px)' }}>
      ...
      <div style={{ padding: '16px 26px 0', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'relative', zIndex: 2 }}>
        <div>
      ...
      <div style={{ flex: 'none', padding: '12px 26px calc(22px + env(safe-area-inset-bottom, 0px))', position: 'relative', zIndex: 2 }}>
        <div style={{ background: '#fff', borderRadius: 30, height: 66, display: 'flex', alignItems: 'center', justifyContent: 'space-around', boxShadow: '0 10px 26px -8px rgba(74,58,102,.28)' }}>
          <div style={{ width: 50, height: 50, borderRadius: '50%', background: 'var(--violet)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><RawSvg html={iconHome} /></div>
          <button onClick={onGoCommunity} style={navBtn}><RawSvg html={iconCommunity} /></button>
          <button onClick={onGoCreate} style={navBtn}><RawSvg html={iconPlus} /></button>
